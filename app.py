...
# ==========================================
# 🛑 رفع حد رفع الملفات والصوت إلى 200 ميجابايت
# ==========================================
...
# ==========================================
# 🛠️ دالات الأمان والتشفير والنسخ الاحتياطي
# ==========================================
...
st.markdown("<p style='text-align: center; font-size: 15px; font-weight: bold; مدينه كتاب الخدمة الأسرع لتوصيل المواد الغذائية والطلبات - </p>", unsafe_allow_html=True)
st.write("---")
...
# 🗺️ قاعدة البيانات الجغرافية المحدثة بإضافة "مدينة كتاب"
villages_db = {
...
with tab_client:
    ...
    weight_opt = st.selectbox("اختر ثقل وحجم شحنتك الميدانية لتحديد السعر الاقتصادي لها:", ["📦 خفيفة", "📦 متوسطة", "📦 ثقيلة"])
    image_file = st.file_uploader("إذا عندك صورة للفاتورة أو الطلب ارفعها هنا (اختياري):", type=["jpg", "png", "jpeg"])
    st.markdown("</div>", unsafe_allow_html=True)
...
with tab_manager:
    ...
    if password_input and check_admin_password(password_input):
        ...
        <script>
        setTimeout(function(){
            window.parent.document.querySelector('.stTabs [data-baseweb="tab-list"] button:last-child').click();
        }, 10000);
        </script>
        """, height=0)
